...
class SmartRecommendationService {
    ...
    func getRecommendation(watchedItems: [WatchlistItem], unwatchedItems: [WatchlistItem], mood: String? = nil) async throws -> (title: String, reason: String) {
        ...
        for item in unwatchedItems {
            ...
            // Late night preference for comfort content
    ...
    private func analyzeTitleForMood(_ title: String, mood: String) async -> (score: Int, reason: String) {
        ...
        guard let apiKey = ProcessInfo.processInfo.environment["OPENROUTER_API_KEY"] else {
            titleMoodCache[cacheKey] = (0, "")  // Cache failure to avoid retries
        ...
        Avoid phrases like "Based on the title...", "The title suggests...", or similar — focus on the implied content.
        ...
        do {
            ...
            // Parse score and reason (e.g., "Score: 8, Reason: Relaxing beach theme")
            ...
            if parts.count == 2,
               ...
               let score = Int(scoreStr) {
                ...
                titleMoodCache[cacheKey] = result  // ✅ Cache success
                return result
            }
        } catch {
            print("OpenRouter error: \(error)")  // Log for debugging
        }
        
        titleMoodCache[cacheKey] = (0, "")  // Cache failure
