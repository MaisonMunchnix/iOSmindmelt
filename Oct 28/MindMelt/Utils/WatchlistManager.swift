...
//            }
    ...
    public func updateItemInSupabase(_ item: WatchlistItem) async {
    ...
    public func deleteItemFromSupabase(_ item: WatchlistItem) async {
        guard supabase.isAuthenticated, item.userId != nil else {
            return
        }
    ...
    // Add to WatchlistManager.swift

    // MARK: - Smart Recommendation (No API Required)
    func getAIRecommendation(mood: String? = nil) async -> (item: WatchlistItem?, reason: String?) {
        print("🔍 WatchlistManager: Received mood: \(mood ?? "nil")")

           let watchedItems = items.filter { $0.isWatched }
           let unwatchedItems = items.filter { !$0.isWatched }
           
           guard !unwatchedItems.isEmpty else {
               return (nil, "No unwatched items available")
           }
           
           do {
               print("🤖 Analyzing your watchlist...")
               let result = try await SmartRecommendationService.shared.getRecommendation(
                   watchedItems: watchedItems,
                   unwatchedItems: unwatchedItems,
                   mood: mood  // Pass the mood here
               )
               ...
               let recommendedItem = unwatchedItems.first { item in
                   item.title.lowercased() == result.title.lowercased()
               }
               ...
               if let item = recommendedItem {
                   ...
                   return (getSmartRandomFallback(unwatchedItems), result.reason)
               }
               
           } catch {
               print("❌ Recommendation error: \(error.localizedDescription)")
               return (getSmartRandomFallback(unwatchedItems), "Smart pick based on time of day")
           }
       }

       private func getSmartRandomFallback(_ unwatchedItems: [WatchlistItem]) -> WatchlistItem? {
           let hour = Calendar.current.component(.hour, from: Date())
           
           // Time-based filtering
           let timeAppropriateItems: [WatchlistItem]
           if hour >= 20 || hour <= 6 {
               // Evening/night - prefer longer content
               timeAppropriateItems = unwatchedItems.filter { $0.category == .long }
           } else {
               // Day time - prefer quicker content
               timeAppropriateItems = unwatchedItems.filter { $0.category == .quick }
           }
           
           return timeAppropriateItems.isEmpty ? unwatchedItems.randomElement() : timeAppropriateItems.randomElement()
       }
